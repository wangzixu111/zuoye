...
class PostsController extends AdminAuth
{
        ...
        if(!empty($param)){
            ...
            if ((isset($param['start_time']) && $param['start_time'] != '') || (isset($param['end_time']) && $param['end_time'] != '')) {
                ...
                if (isset($param['start_time']) && !$param['end_time']) {
                    $map['create_time'] = ['>=', strtotime($param['start_time'])];
                }
    ...
    public function create()
    {
        ...
        $this->data['edit_fields'] = array(
            ...
            'status'         => array('type' => 'radio', 'label' => '状态','default'=> array(-1 => '删除', 1 => '国内新闻', 2 => '本地新闻',3 => '国际新闻')),
            'hr1'            => array('type' => 'hr'),
    ...
    /**
     * [add 新增新闻数据ACTION，create()页面表单数据提交到这里]
     * @return [type] [description]
     */
    public function add()
    {
        ...
        if ($id = $posts->validate(true)->insertGetId($data)) {
            return $this->success('新闻添加成功',$this->data['module_url'].$id);
        } else {
